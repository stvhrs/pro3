...
const BLANK_MASTER_DATA = {
  ...
  judulProject: 'Sosialisasi Kubu Raya',

  namaPekerjaan: 'Pengadaan Jasa Event Organizer (EO) Dalam Rangka Sosialisasi Program Makan Bergizi di Kab Kubu Raya Kab Kubu Raya Provinsi Kalimantan Barat Tanggal 1 Februari 2025',
  ...
  alamatInstansi: 'Komplek Gedung E, Komplek Kementerian Pertanian\nJl. Harsono RM No. 3, Ragunan, Pasar Minggu\nJakarta Selatan 12550',

  namaPpk: 'Muhammad Risal S.',

  nipPpk: '11970059500874',

  provinsiKak: 'Kalimantan Barat',

  kabupatenKak: 'Kabupaten Kubu Raya',

  nilaiKak: 'Rp192.686.529,-',

  

  // Data Summary Baru

  jumlahRab: '169.294.350',

  dasarFee: '143.237.266',

  managementFeeLabel: 'FEE MANAGEMENT',

  managementFee: '4.297.118',

  totalBiaya: '173.591.468',

  ppnLabel: 'PPN (11%)',

  ppn: '19.095.061',

  totalKeseluruhan: '192.686.529',

  tampilkanSummary: true
...
const formatTitleCase = (str) => {
  ...
  const acronyms = ['EO', '(EO)', 'DPR', 'RI', 'MBG', 'KAK', 'RAB', 'HPS', 'BOQ', 'PPK', 'NIP', 'NRP'];
  ...
  return trimmed.toLowerCase().split(/\s+/).map((word, index) => {
    ...
    if (word.includes('/')) return word.split('/').map(w => acronyms.includes(w.toUpperCase()) ? w.toUpperCase() : (w.length > 0 ? w.charAt(0).toUpperCase() + w.slice(1) : '')).join('/');
...
const formatTerbilang = (angkaStr) => {
  ...
  let result = angkaTerbilang(num) + ' rupiah';
...
const generateDateText = (input) => {
  ...
  const monthMap = {
      ...
      'agustus': 'Aug', 'august': 'Aug', 'aug': 'Aug',

      'september': 'Sep', 'sep': 'Sep',

      'oktober': 'Oct', 'october': 'Oct', 'oct': 'Oct',

      'november': 'Nov', 'nov': 'Nov',

      'desember': 'Dec', 'december': 'Dec', 'dec': 'Dec'
  ...
  Object.keys(monthMap).forEach(key => {
      ...
      normalizedInput = normalizedInput.replace(regex, monthMap[key]);
  ...
  if (ddmmyyyyMatch) {
      ...
      d = new Date(`${ddmmyyyyMatch[3]}-${ddmmyyyyMatch[2]}-${ddmmyyyyMatch[1]}T00:00:00`);

  } else {

      d = new Date(normalizedInput);

  }



  if (isNaN(d.getTime())) return 'Format tanggal tidak dikenali';
  ...
  let dateWords = capitalize(angkaTerbilang(dateNum));

  if (dateWords.includes(' Belas')) dateWords = dateWords.replace(' Belas', ' belas');

  

  const yearWords = capitalize(angkaTerbilang(yearNum));
  ...
  return `${dayName} tanggal ${dateWords} bulan ${monthName} tahun ${yearWords} (${dd}-${mm}-${yyyy})`;
...
const parseExcelPaste = (text) => {
  ...
  // Membatasi baris kosong maksimal 1 baris (squash multiple empty rows)
  ...
  for (let i = 0; i < rawRows.length; i++) {
    ...
    if (rawRows[i].every(c => !c || c.trim() === '')) {

      consecutiveEmpty++;

      if (consecutiveEmpty <= 1) squashedRows.push(rawRows[i]);

    } else {

      consecutiveEmpty = 0;

      squashedRows.push(rawRows[i]);
  ...
  squashedRows.forEach(r => rawRows.push(r));
  ...
  if (firstSummaryRowIdx > 0) {
    ...
    // Jika tidak ada summary row dari excel, pastikan persis ada 1 baris kosong di paling akhir

    if (rawRows.length > 0 && !rawRows[rawRows.length - 1].every(c => !c || c.trim() === '')) {

        rawRows.push(new Array(maxCols).fill(''));
...
const getPrintCSS = (paperSize, marginTop = 25.4, marginRight = 25.4, marginBottom = 25.4, marginLeft = 25.4) => {
    ...
    return `
      ...
      .table-doc { width: 100%; border-collapse: collapse; font-size: 10pt; }

      .table-doc td { padding-bottom: 0px; vertical-align: top; border: none; }

      

      .table-bordered { width: 99.8%; max-width: 99.8%; border-collapse: collapse !important; border: 0.5pt solid black !important; font-size: 8pt; line-height: 1.3; margin: 0 auto; box-sizing: border-box; }
      ...
      .table-bordered td.summary-cell { font-weight: bold !important; }
...
const DocumentTemplate = ({ project }) => {
          ...
          <tbody suppressContentEditableWarning>
            ...
            {headerData.body.map((row, rowIndex) => {
              ...
              if (nonEmptyCells.length >= 1 && nonEmptyCells.length <= 2) {
                  ...
                  const summaryKeywords = ['TOTAL', 'FEE', 'PPN', 'PAJAK', 'PEMBULATAN','JUMLAH'];
              ...
              const hasVolume = volCols.some(idx => row[idx] && String(row[idx]).trim() !== '');
              ...
              if (isSummaryRow) {
                ...
                if (masterData.tampilkanSummary) return null; // Jika summary manual nyala, skip bawaan paste tabel
                ...
                let displayLabel = summaryLabel;

                const labelUpper = summaryLabel.toUpperCase();

                if (labelUpper.includes('FEE') || labelUpper.includes('MANAGEMENT')) {

                    displayLabel = 'FEE MANAGEMENT';

                } else if (labelUpper.includes('PPN') || labelUpper.includes('PAJAK')) {
                ...
                return (
                  ...
                  <tr key={`tr-summary-${rowIndex}`} className="bg-white keep-together">
                    ...
                    <td colSpan={row.length - 1} className="px-2 py-2 text-center align-middle summary-cell" style={{ fontWeight: 'bold' }}>{displayLabel}</td>

                    <td className="px-2 py-2 text-right align-middle whitespace-nowrap summary-cell" style={{ fontWeight: 'bold' }}>{val || '\u00A0'}</td>
              ...
              return (
                ...
                <tr key={`tr-${rowIndex}`} className={`hover-bg-gray keep-together`}>
                  ...
                  {row.map((cell, cellIndex) => {
                    ...
                    // Logika Pintar Penghapusan "-"

                    if (text === '-') {

                        if (variant !== 'BOQ') {

                            text = ''; // Hapus "-" jika bukan di tabel BOQ

                        } else if (!hasVolume) {

                            text = ''; // Hapus "-" di tabel BOQ jika baris tersebut tidak punya nilai Volume
            ...
            {/* BLOCK SUMMARY MANUAL (Merespons gambar unggahan) */}

            { masterData.tampilkanSummary && (variant === 'RAB' || variant === 'HPS' || variant === 'BOQ') && (

                <>

                  <tr className="bg-white keep-together">

                      <td colSpan={Math.max(1, (headerData.body[0]?.length || 2) - 1)} className="px-2 py-2 text-center align-middle summary-cell" style={{ fontWeight: 'bold' }}>TOTAL KESELURUHAN</td>

                      <td className="px-2 py-2 text-right align-middle whitespace-nowrap summary-cell" style={{ fontWeight: 'bold' }}>{variant === 'BOQ' ? '-' : masterData.totalKeseluruhan}</td>
  ...
  const TtdPpk = () => (
    ...
    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem', pageBreakInside: 'avoid', lineHeight: '1.15', fontSize: '11pt' }}>
      ...
      <div style={{ textAlign: 'center', width: '300px' }}>
        ...
        <p className="mb-0">NRP. <V>{formatNip(masterData.nipPpk)}</V></p>
    ...
    <>
      ...
      <PaperPage id={`page-master-${project.id || 'new'}`} hideHeader={true} paperSize={pSize} margins={marginProps}>
          ...
          <div className="doc-body text-center font-bold" style={{ paddingTop: '15%' }}>
            ...
            <div style={{ fontSize: '12pt', fontWeight: 'bold', marginBottom: '2.5rem', lineHeight: '1.5' }}>
              ...
              <p className="mb-0">MAKAN BERGIZI UNTUK MASYARAKAT</p>

              <p className="mb-0">KEGIATAN PROMOSI TENTANG PEMENUHAN GIZI</p>

              <p className="mb-0">TAHUN ANGGARAN {masterData.tahunAnggaran}</p>
      ...
      <PaperPage hideHeader={true} paperSize={pSize} margins={marginProps}>
        ...
        <div className="doc-body leading-relaxed" style={{ paddingTop: '5%' }}>
            ...
            <table suppressContentEditableWarning className="w-full mb-6" style={{ fontSize: '10pt', lineHeight: '1.15', border: 'none', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                ...
                <tr><td className="align-top pb-1">Hasil (Outcome)</td><td className="align-top pb-1">: </td><td className="align-top pb-1 text-justify">Meningkatnya pemahaman, kesadaran, dan dukungan masyarakat terhadap pelaksanaan Program Makan Bergizi dalam rangka peningkatan pemenuhan gizi masyarakat.</td></tr>
                ...
                <tr><td className="align-top pb-1">Jenis Keluaran (Output)</td><td className="align-top pb-1">: </td><td className="align-top pb-1 text-justify">Terselenggaranya kegiatan Sosialisasi Program Makan Bergizi kepada masyarakat pada 1 (Satu) titik kegiatan yang dilaksanakan oleh Badan Gizi Nasional bersama mitra kerja Komisi IX DPR RI, serta tersusunnya laporan dan dokumentasi kegiatan.</td></tr>
            ...
            <KakContent>
                ...
                <p className="mb-2" style={{ textIndent: '2.5rem' }}>Program Makan Bergizi merupakan salah satu program pemerintah yang bertujuan untuk meningkatkan kualitas hidup masyarakat melalui pemenuhan kebutuhan gizi yang seimbang serta peningkatan kesadaran masyarakat terhadap pentingnya pola konsumsi pangan yang sehat dan bergizi. Keberhasilan pelaksanaan program tersebut tidak hanya bergantung pada kebijakan pemerintah semata, namun juga memerlukan dukungan serta partisipasi aktif dari masyarakat sebagai sasaran utama program.</p>

                <p className="mb-2" style={{ textIndent: '2.5rem' }}>Dalam pelaksanaannya, masih terdapat keterbatasan pemahaman masyarakat mengenai tujuan, manfaat, serta mekanisme pelaksanaan program Makan Bergizi. Kondisi ini menunjukkan bahwa upaya penyebarluasan informasi kepada masyarakat masih perlu terus ditingkatkan agar masyarakat dapat memperoleh informasi yang jelas, benar, dan komprehensif mengenai program Makan Bergizi.</p>

                <p className="mb-2" style={{ textIndent: '2.5rem' }}>Sebagai bagian dari upaya penyebarluasan informasi tersebut, Badan Gizi Nasional (BGN) melaksanakan kegiatan sosialisasi program Makan Bergizi kepada masyarakat melalui pendekatan komunikasi publik secara langsung. Kegiatan sosialisasi ini menjadi salah satu sarana strategis dalam menyampaikan informasi program pemerintah sekaligus membangun kesadaran masyarakat terhadap pentingnya pemenuhan gizi yang baik.</p>

                <p className="mb-2" style={{ textIndent: '2.5rem' }}>Dalam rangka memperkuat jangkauan penyampaian informasi kepada masyarakat, pelaksanaan kegiatan sosialisasi ini juga dilaksanakan melalui kemitraan Badan Gizi Nasional dengan Anggota Komisi IX DPR RI, yang memiliki fungsi strategis. Melalui kemitraan tersebut diharapkan penyebarluasan informasi mengenai program Makan Bergizi dapat menjangkau masyarakat secara lebih luas, khususnya pada wilayah daerah pemilihan.</p>

                <p className="mb-0" style={{ textIndent: '2.5rem' }}>Sehubungan dengan hal tersebut, Badan Gizi Nasional melaksanakan kegiatan sosialisasi program Makan Bergizi kepada masyarakat pada berbagai wilayah. Kerangka Acuan Kerja (KAK) ini disusun sebagai pedoman pelaksanaan kegiatan sosialisasi program Makan Bergizi pada 1 (Satu) titik kegiatan, sehingga pelaksanaan kegiatan dapat berjalan secara efektif, efisien, serta dapat dipertanggungjawabkan secara administratif.</p>
            ...
            <KakContent>
                ...
                <div className="mb-4">
                  ...
                  <p className="mb-0">Menyelenggarakan kegiatan sosialisasi program Makan Bergizi kepada masyarakat sebagai upaya penyebarluasan informasi program pemerintah di bidang gizi kepada masyarakat secara langsung.</p>
                ...
                <div>
                  ...
                  <ol className="mb-0">
                    ...
                    <li>Meningkatkan pemahaman masyarakat mengenai program Makan Bergizi.</li>

                    <li>Menyebarluaskan informasi terkait program Makan Bergizi kepada masyarakat.</li>

                    <li>Meningkatkan kesadaran masyarakat mengenai pentingnya pemenuhan gizi yang baik.</li>

                    <li>Mendorong partisipasi masyarakat dalam mendukung pelaksanaan program Makan Bergizi.</li>
            ...
            <KakContent>
                ...
                <ol className="mb-0">
                  ...
                  <li>Terselenggaranya kegiatan Sosialisasi Program Makan Bergizi kepada masyarakat pada 1 (Satu) titik kegiatan.</li>

                  <li>Tersedianya materi sosialisasi program Makan Bergizi.</li>
            ...
            <KakContent>
                ...
                <ol className="mb-0">
                  ...
                  <li>Meningkatnya pemahaman masyarakat mengenai program Makan Bergizi.</li>

                  <li>Meningkatnya kesadaran masyarakat terhadap pentingnya pemenuhan gizi yang baik.</li>

                  <li>Tersampaikannya informasi program Makan Bergizi kepada masyarakat secara langsung.</li>

                  <li>Meningkatnya dukungan masyarakat terhadap pelaksanaan program Makan Bergizi.</li>
            ...
            <div className="keep-together">
              ...
              <KakContent>
                  ...
                  <table suppressContentEditableWarning className="w-full mb-0" style={{ fontSize: '10pt', border: 'none' }}>
                    ...
                    <tbody>
                      ...
                      <tr><td className="align-top pb-1">Lokasi</td><td className="align-top pb-1">: </td><td className="align-top pb-1">Komplek Gedung E, Komplek Kementerian Pertanian<br/>Jl. Harsono RM No. 3, Ragunan, Pasar Minggu<br/>Jakarta Selatan 12550</td></tr>
            ...
            <div className="keep-together">
              ...
              <KakContent>
                  ...
                  <p className="mb-0">Pembiayaan kegiatan bersumber dari DIPA Badan Gizi Nasional Tahun Anggaran {masterData.tahunAnggaran}, dengan SP DIPA-128.01-0/{masterData.tahunAnggaran} tanggal 2 Desember {parseInt(masterData.tahunAnggaran) - 1};</p>

                  <ol className="mb-0">

                    <li>Akun kegiatan: 7076.AEH.001.052.B.522191</li>
            ...
            <div className="keep-together">
              ...
              <KakContent>
                  ...
                  <p className="mb-2" style={{ textIndent: '2.5rem' }}>Kerangka Acuan Kerja (KAK) ini disusun sebagai pedoman dalam pelaksanaan kegiatan Sosialisasi Program Makan Bergizi kepada masyarakat pada 1 (Satu) titik kegiatan yang dilaksanakan oleh Badan Gizi Nasional melalui kemitraan dengan Anggota Komisi IX DPR RI.</p>

                  <p className="mb-0" style={{ textIndent: '2.5rem' }}>Dengan adanya kegiatan ini diharapkan masyarakat dapat memperoleh informasi yang jelas mengenai program Makan Bergizi sehingga dapat meningkatkan pemahaman, kesadaran, serta dukungan masyarakat terhadap upaya pemerintah dalam meningkatkan status gizi masyarakat.</p>
...
export default function App() {
  ...
  // Auto-Extract Total & Rekapitulasi dari Paste Tabel
  ...
  useEffect(() => {
      ...
      if (pastedKAK) {
          ...
          // Kata Kunci untuk Auto-Sum Biaya Operasional/Dasar Fee
          ...
          rawRows.forEach(row => {
              ...
              if (nonEmpty.length >= 2) {
                  ...
                  const label = String(nonEmpty[0]).toUpperCase();

                  

                  // Cari kolom Biaya Total: pilih cell numerik terakhir untuk hindari kolom Keterangan

                  const numericCells = nonEmpty.filter(c => /^[\sRp.,0-9\-]+$/i.test(String(c).trim()));

                  let valueStr = nonEmpty[nonEmpty.length - 1];

                  if (numericCells.length > 0) {

                      valueStr = numericCells[numericCells.length - 1];

                  }

                  const cleanVal = sanitizeCurrency(valueStr);

                  

                  if (label.includes('KESELURUHAN')) extracted.keseluruhan = valueStr;

                  else if (label.includes('PPN') || label.includes('PAJAK')) extracted.ppn = valueStr;

                  else if (label === 'TOTAL') extracted.total = valueStr;

                  else if (label.includes('FEE') || label.includes('MANAGEMENT')) extracted.fee = valueStr;

                  else if (label === 'JUMLAH' || label === 'JUMLAH BIAYA' || label === 'SUBTOTAL') extracted.jumlah = valueStr;

                  else {

                      // Cek apakah baris ini termasuk ATK / Dokumentasi untuk di-Sum

                      const rowText = nonEmpty.join(' ').toUpperCase();

                      if (targetItems.some(kw => rowText.includes(kw))) {

                          sumDasarFee += cleanVal;
          ...
          setMasterData(prev => {
              ...
              if (extracted.jumlah) { nextState.jumlahRab = extracted.jumlah; updated = true; }

              if (extracted.fee) { nextState.managementFee = extracted.fee; updated = true; }

              if (extracted.total) { nextState.totalBiaya = extracted.total; updated = true; }

              if (extracted.ppn) { nextState.ppn = extracted.ppn; updated = true; }

              if (extracted.keseluruhan) { 

                  nextState.totalKeseluruhan = extracted.keseluruhan; 

                  const cleanTotal = extracted.keseluruhan.replace(/[^0-9]/g, '');

                  if (cleanTotal && cleanTotal !== '0') nextState.nilaiKak = formatCurrencyDisplay(cleanTotal);

                  updated = true; 

              } else if (extracted.total) {

                  const cleanTotal = extracted.total.replace(/[^0-9]/g, '');
              ...
              // Set hasil Auto-Sum ATK ke dalam Dasar Pengenaan Fee

              if (sumDasarFee > 0) {

                  nextState.dasarFee = new Intl.NumberFormat('id-ID').format(sumDasarFee).replace(/,/g, '.');
              ...
              if (updated) {
                  ...
                  // Paksa label fee murni tanpa persentase secara default

                  nextState.managementFeeLabel = 'FEE MANAGEMENT';



                  // Kunci PPN di 11% sesuai permintaan

                  nextState.ppnLabel = 'PPN (11%)';
  ...
  const sortProjectsDescByTitleNumber = (a, b) => {

      const titleA = a.masterData?.judulProject || a.masterData?.namaPekerjaan || '0';

      const titleB = b.masterData?.judulProject || b.masterData?.namaPekerjaan || '0';

      

      const numA = parseInt(titleA, 10) || 0;

      const numB = parseInt(titleB, 10) || 0;

      

      // Jika angkanya sama, fallback ke pengurutan string biasa
      ...
      return numB - numA; // Sortir Descending (Terkecil/Terbesar)
  ...
  const filteredProjects = useMemo(() => {
      ...
      // 2. Mode Pencarian Rentang Angka (Misal: 371 s.d 420)

      const startNum = parseInt(searchRangeStart, 10);

      const endNum = parseInt(searchRangeEnd, 10);
      ...
      if (!isNaN(startNum) && !isNaN(endNum)) {
          ...
          filtered = filtered.filter(p => {
              ...
              const title = p.masterData?.judulProject || p.masterData?.namaPekerjaan || '';

              const numMatch = title.match(/\d+/); // Ambil angka pertama dari judul

              if (numMatch) {

                  const projNum = parseInt(numMatch[0], 10);

                  return projNum >= min && projNum <= max;

              }

              return false; // Jika tidak ada angka, abaikan
      ...
      // Sortir berdasarkan angka di judul (Descending: Terbesar ke Terkecil)

      filtered.sort(sortProjectsDescByTitleNumber);

      return filtered;

  }, [projects, searchQuery, searchRangeStart, searchRangeEnd]);
  ...
  const handleDateInput = (e) => {
    ...
    if (!val || val.trim() === '') {
        ...
        const hasil = generateDateText(val);
  ...
  const handleCopyDate = () => {
    ...
    if (!dateOutput || dateOutput === 'Format tanggal tidak dikenali') return;

    const textArea = document.createElement("textarea");

    textArea.value = dateOutput;
    ...
    try {
      ...
      messageApi.success('Teks tanggal KAK berhasil disalin ke Clipboard!');
      ...
      {view === 'home' ? (
        ...
        <Layout style={{ minHeight: '100vh', background: '#f5f5f5' }}>
            ...
            <Card style={{ marginBottom: 24, borderRadius: 8, border: '1px solid #91d5ff' }} bodyStyle={{ padding: '16px 24px', background: '#e6f7ff' }}>
              ...
              <Row gutter={[16, 16]} align="middle">
                ...
                <Col xs={24} md={8}>
                  ...
                  <Text strong style={{ display: 'block', marginBottom: 8, fontSize: 12, textTransform: 'uppercase', color: '#096dd9' }}>Alat Bantu 1: Konversi Terbilang Uang</Text>
              ...
              <Divider style={{ borderColor: '#bae0ff', margin: '16px 0' }} />
              ...
              <Row gutter={[16, 16]} align="middle">
                ...
                <Col xs={24} md={8}>
                  ...
                  <Text strong style={{ display: 'block', marginBottom: 8, fontSize: 12, textTransform: 'uppercase', color: '#096dd9' }}>Alat Bantu 2: Format Tanggal KAK</Text>

                  <Input placeholder="Contoh: 03 February 2026 atau 14 Januari 2026" value={dateInput} onChange={handleDateInput} allowClear />
                ...
                <Col xs={24} md={16}>
                   ...
                   <div 
                     ...
                     onClick={dateOutput && dateOutput !== 'Format tanggal tidak dikenali' ? handleCopyDate : undefined}

                     title={dateOutput && dateOutput !== 'Format tanggal tidak dikenali' ? "Klik di mana saja pada area ini untuk menyalin teks" : ""}
                     ...
                     style={{ 
                       ...
                       cursor: (dateOutput && dateOutput !== 'Format tanggal tidak dikenali') ? 'pointer' : 'default',

                       transition: 'background-color 0.2s'

                     }}

                     onMouseEnter={(e) => { if(dateOutput && dateOutput !== 'Format tanggal tidak dikenali') e.currentTarget.style.backgroundColor = '#f0f5ff'; }}

                     onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = '#ffffff'; }}

                   >

                     <Text style={{ color: dateOutput === 'Format tanggal tidak dikenali' ? '#cf1322' : '#0050b3', margin: 0, flex: 1, userSelect: 'none' }}>

                       {dateOutput || "Hasil teks tanggal KAK akan muncul di sini..."}

                     </Text>

                     {dateOutput && dateOutput !== 'Format tanggal tidak dikenali' && (
            ...
            <Card style={{ marginBottom: 24, borderRadius: 8 }} bodyStyle={{ padding: '16px 24px' }}>
              ...
              <Row gutter={[16, 16]} align="bottom">
                ...
                <Col xs={24} md={10}>
                  ...
                  <Input prefix={<Search size={16} color="#bfbfbf" />} placeholder="Ketik kata kunci..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} allowClear />

                </Col>

                <Col xs={24} md={14}>

                  <Text strong style={{ display: 'block', marginBottom: 8, fontSize: 12, textTransform: 'uppercase' }}>Filter Rentang Nomor (Contoh: 371 s.d 420)</Text>

                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>

                    <Input type="number" placeholder="No. Awal" value={searchRangeStart} onChange={e => setSearchRangeStart(e.target.value)} style={{ flex: 1 }} allowClear />

                    <span style={{ color: '#8c8c8c', fontSize: 12, fontWeight: 600 }}>S.D</span>

                    <Input type="number" placeholder="No. Akhir" value={searchRangeEnd} onChange={e => setSearchRangeEnd(e.target.value)} style={{ flex: 1 }} allowClear />
          ...
          <div id="bulk-print-container" style={{ display: 'none' }}>
             ...
             {isBulkPrinting && projects

                 .filter(p => selectedIds.includes(p.id))

                 .sort(sortProjectsDescByTitleNumber)
              ...
              <div style={{ width: 500, height: '100%', overflowY: 'auto', overflowX: 'hidden' }}>
                ...
                <div style={{ padding: '0 16px 24px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: 16 }}>
                    ...
                    {/* Card 3: Rekapitulasi Anggaran (Summary Gambar) */}

                    <Card size="small" title={

                        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%'}}>

                            <span style={{fontWeight: 'bold', color: '#1f2937'}}>Rekapitulasi Anggaran (Summary)</span>

                            <Checkbox 

                                checked={masterData.tampilkanSummary} 

                                onChange={e => setMasterData(prev => ({...prev, tampilkanSummary: e.target.checked}))}
                        ...
                        {masterData.tampilkanSummary && (

                            <Row gutter={12}>

                                <Col span={24}><FormGroup label="Total Keseluruhan"><Input name="totalKeseluruhan" value={masterData.totalKeseluruhan} onChange={handleMasterDataChange} /></FormGroup></Col>
                    ...
                    <Card size="small" title={<span style={{fontWeight: 'bold', color: '#1f2937'}}>Data PPK (Pejabat Pembuat Komitmen)</span>}>
                      ...
                      <Row gutter={12}>
                        ...
                        <Col span={12}><FormGroup label="NIP/NRP PPK"><Input name="nipPpk" value={masterData.nipPpk} onChange={handleMasterDataChange} /></FormGroup></Col>
            ...
            >
              ...
              <div style={{ width: 350, height: '100%', overflowY: 'auto', overflowX: 'hidden', display: 'flex', flexDirection: 'column' }}>
                 ...
                 <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
                    ...
                    {[...projects].sort(sortProjectsDescByTitleNumber).map(proj => (
